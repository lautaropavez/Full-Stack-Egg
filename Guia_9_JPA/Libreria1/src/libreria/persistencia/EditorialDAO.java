...
public class EditorialDAO {
    ...
    public Editorial buscarEditorialPorNombre(String nombre) throws Exception {
        try {
            Editorial editorial = (Editorial) em.createQuery("SELECT e "
                + " FROM Editorial e"
                + " WHERE e.nombre LIKE :nombre"). //+ " WHERE e.nombre LIKE CONCAT('%', :nombre, '%')", Editorial.class).
                setParameter("nombre", nombre).    //setParameter("nombre", nombre)
                getSingleResult();
    ...
    public void eliminarEditorialPorId(String id) throws Exception {
        try {
            ...
            throw new Exception("Error al eliminar Editorial por ID.");
        }
    }
