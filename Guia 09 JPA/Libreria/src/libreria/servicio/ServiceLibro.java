...
public class ServiceLibro {
    ...
    public void crearLibro(String titulo, Integer anio, String nombreAut, String nombreEdit) throws Exception {
        try {
            //Validamos
            //Fijarme si debo corroborar con otro método que no exista el titulo
...
//    }
    ...
    public void eliminarPorEditorial(String nombEditorial) throws Exception {
        try {
            ...
            if (libros.isEmpty()) {
                System.out.println("La editorial ingresada no está asociada a ningún libro");
//                throw new Exception("La editorial ingresada no está asociada a ningún libro");
    ...
    public void eliminarPorAutor(String nombAutor) throws Exception {
        try {
            // VALIDACIÓN
            if (nombAutor == null || nombAutor.trim().isEmpty()) {
                throw new Exception("Debe indicar el Autor");
            }
            //Buscamos
            Autor autor = servAut.buscarPorNombre(nombAutor);
            List <Libro> libros = daolibro.buscarPorAutor(nombAutor);
            
            if (autor == null) {
                throw new Exception("El autor ingresado no existe");
            }
            if (libros.isEmpty()) {
                throw new Exception("El autor ingresado no está asociado a ningún libro");
            }else{
                //Eliminamos
                daolibro.eliminaPorAutor(autor);
                System.out.println("LIBROS ELIMINADOS CON EXITO");
            }   
        } catch (Exception e) {
            throw e;
    ...
    public void listaPorAnio(Integer anio) throws Exception {
        try {
            ...
            if (buscado.isEmpty()) {
                throw new Exception("NO EXISTEN LIBROS DE ESTE AÑO");
            } else {
                System.out.println("\n*** LISTA DE LIBROS ***");
    ...
    public void listarLibros() throws Exception { 
        ...
        try {
            ...
            // System.out.println(e.getMessage()); //No lo pongo porque si lo pongo salta la e dos ves que seria dos veces el mensaje "NO EXISTEN LIBROS"
            
        }
    }
    
//---------------------------- OTRAS MANERAS DE HACER METODOS ------------------
    ...
    //OTRA MANERA DE LISTAR (EN VEZ DE PONER LOS SOUT PARA MOSTRAR EN EL SERVICE LO HACEMOS EN EL MENÚ) ACLARACIÓN: NO ESTÁ HECHO EL METODO CON LOS SOUTS EN EL MENU PARA USAR ESTE METODO    
    //ESTE METODO LO USO TAMBIEN EN EL SERVICIO EDITORIAL
